...
class _DenseLayer(nn.Sequential):
    ...
    def __init__(self, num_input_features, growth_rate, bn_size, dilation, checkpointing=True):
        ...
        self.add_module("norm2", nn.BatchNorm2d(bn_size * growth_rate, momentum=batchnorm_momentum))
        ...
        if checkpointing:
            self.func = _DenseLayer._checkpoint_function_factory(
...
class _DenseBlock(nn.Sequential):
    def __init__(self, num_layers, num_input_features, bn_size, growth_rate, dilation=1, checkpointing=True):
        super(_DenseBlock, self).__init__()
        for i in range(num_layers):
            layer = _DenseLayer(
                num_input_features + i * growth_rate, growth_rate, bn_size, dilation, checkpointing=checkpointing
            )
...
class _Transition(nn.Sequential):
    ...
    def __init__(self, num_input_features, num_output_features, checkpointing=True):
        ...
        self.add_module(
            "pool", nn.AvgPool2d(kernel_size=3, stride=2, padding=1, ceil_mode=False, count_include_pad=False)
        )
...
class _BNReluConv(nn.Sequential):
    ...
    def __init__(self, num_maps_in, num_maps_out, k=3, batch_norm=True, bias=False, dilation=1, checkpointing=True):
        ...
        padding = k // 2
        self.add_module(
            "conv", nn.Conv2d(num_maps_in, num_maps_out, kernel_size=k, padding=padding, bias=bias, dilation=dilation)
        )
...
class SpatialPyramidPooling(nn.Module):
    def __init__(
        self, num_maps_in, num_levels, bt_size=512, level_size=128, out_size=256, grids=[6, 3, 2, 1], square_grid=False
    ):
    ...
    def forward(self, x, drop_rate=0):
        ...
        for i in range(1, num):
            ...
            level = F.interpolate(level, target_size, mode="bilinear")
            levels.append(level)
...
class DenseNet(nn.Module):
    def __init__(self, growth_rate=32, block_config=(6, 12, 32, 32), num_init_features=64, bn_size=4):
...
class Ladder(nn.Module):
    def __init__(self, args, num_classes=19):
        ...
        self.spp = SpatialPyramidPooling(
            num_features, num_levels, bt_size, level_size, self.spp_size, spp_grids, spp_square_grid
        )
        ...
        for i in range(len(self.backbone.skip_sizes)):
            upsample = _Upsample(
                num_features, self.backbone.skip_sizes[-1 - i], up_sizes[i], num_classes=self.num_classes
            )
