...
class TwoHead(model_utils.DNN):
    def __init__(self, args, num_classes=19, ignore_id=None):
        ...
        for i in range(len(self.feature_extractor.backbone.skip_sizes)):
            self.aux_logits.add_module(
                "upsample_" + str(i),
                layers._BNReluConv(
                    self.feature_extractor.upsample_layers[i].num_maps_in, self.num_classes, k=1, bias=True
                ),
            )
            self.aux_logits_ood.add_module(
                "upsample_" + str(i),
                layers._BNReluConv(self.feature_extractor.upsample_layers[i].num_maps_in, 2, k=1, bias=True),
            )
    ...
    def forward(self, x, target_size=None):
        ...
        self.out = F.interpolate(x, target_size, mode="bilinear", align_corners=False)
        out_conf = F.interpolate(x_conf, target_size, mode="bilinear", align_corners=False)
        ...
        for name, param in state_dict.items():
            if convert:
                ...
                if "aux_logits" in name:
                    name = (
                        name.replace("upsample_layers.", "")
                        .replace("aux_logits.", "")
                        .replace("upsample", "aux_logits.upsample")
                    )
