...
class DatasetReader(Dataset):
    def __init__(self, args):
        ...
        self.crops = []
        for f in files:
            annotation = ET.parse(join(pascal_dir, "Annotations", f.replace(".png", ".xml")))
            anno_iterator = annotation.getiterator()
            objects = [
                (child.tag, child.text)
                for child in anno_iterator
                if child.tag in ["name", "xmax", "xmin", "ymax", "ymin"]
            ][1:]
            filtered_objects = [
                (
                    i + 1,  # object index
    ...
    def __getitem__(self, idx):
        ...
        img_wd[h_start : h_start + h_pascal, w_start : w_start + w_pascal][label_pascal == 1] = img_pascal[
            label_pascal == 1
        ]
        ...
        img_wd.save("./outputs/img/wd-pascal/" + str(idx) + "_img.png")
        # labels.save("./outputs/wd-pascal/" + str(idx) + "_label.png")

        img_wd = np.array(img_wd, dtype=np.float32)
        labels = np.array(labels, dtype=np.uint8)

        np.save("./outputs/label/wd-pascal/" + str(idx) + "_label", labels)

        batch["mean"] = self.mean
        batch["std"] = self.std
        img_wd = transform.normalize(img_wd, self.mean, self.std)

        img_wd = transform.numpy_to_torch_image()
        batch["image"] = img_wd
